...
function sendFormData(name, email, message) {
  ...
  // Заголовок Content-Type с кодировкой для серв.
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      // пишем что будет выполняться при получении ответа с сервера
      // (readyState:4: запрос завершен и ответ готов, status:200: "OK (все хорошо)")
      console.log(xhr);
    }
  };
